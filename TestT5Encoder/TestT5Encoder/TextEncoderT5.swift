...
//            token == padToken ? maskToken : padToken // existing
            token == padToken ? padToken : maskToken // fixed
        }
      
//        attentionMask[0] = bosToken // existing
        attentionMask[0] = eosToken // fixed
        print(inputIds)
        print(attentionMask)
        ...
        var output = [Float]()
        for i in 0...100 {
              output.append(embeddingFeature?.multiArrayValue![i] as! Float)
          }
        print(output[0...100])
